...
class PostProcess(nn.Module):
    ...
    def __init__(
        ...
        nms_iou_threshold=-1,
        confidence_score=-1,
        temperature=1.0  # 추가: temperature 인자
    ):
        super().__init__()
        self.select_box_nums_for_evaluation = select_box_nums_for_evaluation
        self.nms_iou_threshold = nms_iou_threshold
        self.confidence_score = confidence_score
        self.temperature = temperature  # 온도 초기화
    ...
    def forward(self, outputs, target_sizes):
        ...
        prob = F.softmax(out_logits / self.temperature, dim=-1)  # sigmoid 대신 softmax + temperature scaling
        ...
        if torchvision._is_tracing():
            # avoid iteration warning during ONNX export
